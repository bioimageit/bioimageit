...
class PyFlow(QMainWindow):
	...
	def manageUnexecutedNodesData(self):
		unexecutedNodesWhichHaveData = [node for node in self.graphManager.get().getAllNodes() if hasattr(node, 'executed') and node.executed and hasattr(node, 'hasGeneratedData') and node.hasGeneratedData()]
		if len(unexecutedNodesWhichHaveData) > 0:
			answer = QMessageBox.warning(self, "Delete data?", f'Some data was generated by the current node. This data must be deleted to continue. Export the workflow with its data before saving if you want to keep the current state of your work. Would you like to send it to the trash?', QMessageBox.Cancel | QMessageBox.Yes)
	...
	def save(self, save_as=False):
		...
		if not self.currentFileName == "":
			# if not self.manageUnexecutedNodesData(): return False
