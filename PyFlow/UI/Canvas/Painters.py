...
class NodePainter(object):
    ...
    def default(node, painter, option, widget):
        ...
        color = QtGui.QColor(255, 255, 255) # node.color
        ...
        # pen = QtGui.QPen(QtCore.Qt.white, 0.25)
        ...
        # Label is header background color
        br = QtGui.QBrush()
        painter.setBrush(br)
        if node.drawlabel:
            lr = QtCore.QRectF(r)
        ...
        if not node.isValid():
            ...
            pen.setWidthF(1) # pen.width() * 1.5)
        elif not node.bExposeInputsToCompound:
            if option.state & QStyle.State_Selected:
                # pen.setColor(editableStyleSheet().MainColor)
                pen.setColor(Colors.SelectionBlue)
                pen.setStyle(node.optPenSelectedType)
                pen.setWidthF(2) # pen.width() * 4.5)
        else:
            pen.setColor(ExposedPropertiesColor)
            pen.setStyle(node.optPenSelectedType)
            pen.setWidthF(1) # pen.setWidth(pen.width() * 1.5)
