...
class UINodeBase(QGraphicsWidget, IPropertiesViewSupport, IUINode):
    ...
    def __init__(
        ...
        self._labelTextColor = QtCore.Qt.black
    ...
    def noPadding(self, pwidget=None, playout=None):
        playout = ( pwidget.layout if isinstance(pwidget.layout, QLayout) else pwidget.layout() ) if playout is None else playout
        if playout is None: return
        for i in range(playout.count()):
            widget = playout.itemAt(i).widget()
            if widget is not None:
                widget.setStyleSheet('margin: 0; padding: 0; border:1px solid red;')
                self.noPadding(widget)
            layout = playout.itemAt(i).layout()
    ...
    def createPropertiesWidget(self, propertiesWidget):
        ...
        Info = CollapsibleFormWidget(headName="Info", collapsed=False, hideLabels=True)
        doc = QTextBrowser()
    ...
    def createBaseWidgets(self, propertiesWidget):
        # baseCategory = CollapsibleFormWidget(headName="Base", collapsed=True)
