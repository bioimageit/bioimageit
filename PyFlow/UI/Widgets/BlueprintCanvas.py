...
class BlueprintCanvas(CanvasBase):
    ...
    def connectTools(self):
        self.requestFillProperties.connect(
            self.pyFlowInstance.onRequestFillProperties
        )
        self.requestFillTable.connect(
            self.pyFlowInstance.onRequestFillTable
        )
        self.requestClearProperties.connect(
            self.pyFlowInstance.onRequestClearProperties
        )
        self.requestClearTable.connect(
            self.pyFlowInstance.onRequestClearTable
        )
    
    def disconnectTools(self):
        self.requestFillProperties.disconnect()
        self.requestFillTable.disconnect()
        self.requestClearProperties.disconnect()
        self.requestClearTable.disconnect()
    ...
    def mouseReleaseEvent(self, event):
        ...
        if all(
            ...
            if self.released_item is not None:
                ...
                self.tryFillTableView(pressedNode)
    ...
    def tryFillTableView(self, obj):
        if isinstance(obj, IPropertiesViewSupport):
            self.requestFillTable.emit(obj)
